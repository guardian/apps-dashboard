...
grep -o '#....' git_history.txt | grep -o '[0-9][0-9][0-9][0-9]' | xargs -I {} curl --silent --user "${GITHUB_CREDENTIALS}" https://api.github.com/repos/guardian/android-news-app/pulls/{} | jq -r '.title + " <a href=\"" + ._links.html.href + "\">(#" + (.number|tostring) + ")</a><br>"' > pr_subjects.txt
...
grep -o AA-[0-9][0-9][0-9] git_history.txt | xargs -I{} curl -s -u ${JIRA_CREDENTIALS} https://theguardian.atlassian.net/rest/api/latest/issue/{} | jq -r '.key + "+" + .fields.issuetype.name + "+" + .fields.resolution.name + "+" + .fields.summary + " <a href=\"https://theguardian.atlassian.net/browse/" + .key + "\">(" + .key + ")</a><br>"' > jira_tickets.txt
