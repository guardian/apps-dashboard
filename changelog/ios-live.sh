...
grep -o -E "1\.0\.[0-9]*" git_history.txt | xargs -I {} curl --silent --user "${GITHUB_CREDENTIALS}" https://api.github.com/repos/guardian/mobile-apps-article-templates/git/refs/tags/{} | jq -r '.object.url' | xargs curl --user "${GITHUB_CREDENTIALS}" --silent | jq -r '.object.url' | xargs curl --user "${GITHUB_CREDENTIALS}" --silent | jq '.message' | grep -o '#[0-9]*' | tr -d '#' | xargs -I {} curl --silent --user "${GITHUB_CREDENTIALS}" https://api.github.com/repos/guardian/mobile-apps-article-templates/pulls/{} | jq -r '.title + " <a target=\"_blank\" href=\"" + ._links.html.href + "\">(#" + (.number|tostring) + ")</a><br>"' > templates.txt
