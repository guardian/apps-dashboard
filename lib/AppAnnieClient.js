...
AppAnnieClient.prototype.getGooglePlayStarRating = function (callback) {
	...
	var reviews = [];
AppAnnieClient.prototype.getGooglePlayReviews = function (startDate, endDate, pageIndex, callback) {
	var url = 'https://api.appannie.com/v1.2/apps/google-play/app/' + module.androidProductID + '/reviews?start_date=' + startDate + '&end_date=' + endDate + '&page_index=' + pageIndex;
	...
	callAPI(url, (err, result) => {
		...
		if (result.next_page)
			this.getGooglePlayReviews(startDate, endDate, result.page_index + 1, callback)
		else
