...
GuardianApp.getLatestGooglePlayReleases = function(callback) {
	this.getGooglePlayTracks((err, tracks) => {
		if(err) callback(err);
		async.map(tracks, (track, cb) => {
			var versionCode = track["versionCodes"][0];
			var lane = track["track"];
			this.getReleaseDataFromGithub(versionCode, (err, data) => {
				var release = {
					track: lane,
					data: {
				...
				if(lane === "rollout")
					release.data["userFraction"] = track.userFraction;

				cb(null, release);
			});
		}, (err, result) => {
			callback(err, result.reduce((sum, e) => {sum[e.track] = e.data;return sum}, {}))} );
	});
}
