...
report.request('Report.GetMetrics', reportData, function (err, response) {
    ...
    report.request('Report.GetElements', reportData, function (err, response) {
        ...
        var reportSuiteIDs = {
            "rsid_list": ["guardiangu-globalapps-prod"]
        };
        report.request('ReportSuite.GetSegments', reportSuiteIDs, function (err, response) {
            if (err) throw err;
            console.log("Segments:");
            //console.log(JSON.stringify(response));
            response[0].segments.forEach(function (elem) {
                console.log(elem.name + ":" + elem.id);
            });
        });
    });
